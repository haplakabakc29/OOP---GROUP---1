...
public class browsePage extends JFrame implements ActionListener {
    ...
    public void addToCart(String name, int price, int quantity) {
        for (int i = 0; i < quantity; i++) {
            if (itemCount < cartItems.length) {
                cartItems[itemCount] = name;
                cartPrices[itemCount] = price;
                itemCount++;
            } else {
                JOptionPane.showMessageDialog(this, "Cart is full! Cannot add more items.");
                break;
            }
        }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnViewCart) {
            new CheckoutPage(cartItems, cartPrices).setVisible(true);
        } else if (e.getSource() == btnView1) {
            new productDetails("Wireless Earbuds", 1299, "Compact earbuds with clear sound and long \nbattery life", this).setVisible(true);
        } else if (e.getSource() == btnView2) {
            new productDetails("Smart Watch", 2499, "Tracks your steps, heart rate, and notifications \non your wrist", this).setVisible(true);
        } else if (e.getSource() == btnView3) {
            new productDetails("Phone Stand", 349, "Adjustable desk stand compatible with any \nsmartphone", this).setVisible(true);
        } else if (e.getSource() == btnView4) {
            new productDetails("Portable Charger", 899, "10,000mAh power bank that charges \nyour devices on the go", this).setVisible(true);
        } else if (e.getSource() == btnView5) {
            new productDetails("USB Hub", 599, "4-port USB hub to connect multiple devices \nat once", this).setVisible(true);
        } else if (e.getSource() == btnView6) {
            new productDetails("LED Desk Lamp", 749, "Touch-controlled lamp with adjustable brightness \nlevels", this).setVisible(true);
        }
